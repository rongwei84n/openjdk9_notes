...
2. hash = hash ^ (hash >>> 16) 高16位和低16亦或操作
3. (n-1) & hash得到正确的槽位，不用hash % (n-1)求余是为了更高的性能，其实结果是一样的。
...
如果你去看Object的默认hashcode方法，一般是返回31位，而经典的String对hashcode进行了重写。

如果字符串短，那么返回的hashcode也短，如果长那就变长，最长32位。
